...
async function initApp() {
    ...
    loadUsersFromLocalStorage(); // Load all users
 ...
 * @param {Array} users An array of user objects to save.
 */
...
/**
 * Checks if a user is logged in and loads their profile.
 */
function checkLoginStatus() {
    const loggedInUsername = localStorage.getItem(LS_LOGGED_IN_USER_KEY);
    if (loggedInUsername) {
        ...
        if (currentUser) {
            ...
            console.warn("Logged in user not found in local storage. Clearing login status.");
        }
    }
    // If not logged in or invalid user, show login modal
...
async function handleRegister(event) {
    ...
    if (users.some(u => u.username === username || u.email === email)) {
        ...
        const newUser = {
            ...
            username,
            email,
            password, // Storing plain text for frontend-only demo; NEVER do this in real backend
        ...
        showPage('dashboard'); // Go to dashboard after registration
    }
    hideLoading();
}
...
function updateUI() {
    ...
    document.getElementById('totalScans').innerText = user.pointsHistory.filter(item => item.action === 'qr_scan').length;
    document.getElementById('totalRedeemed').innerText = user.redeemedCoupons.length;

    renderRecentActivity(user.pointsHistory || []);
    renderPointsHistory(user.pointsHistory || [], user.redeemedCoupons || []);
}
...
async function handleQRScan(decodedText) {
    ...
    try {
        ...
        // --- Simulate backend logic for QR scanning ---
        let users = loadUsersFromLocalStorage();
        ...
        currentUser.pointsHistory.push({
            action: 'qr_scan',
            points: pointsEarned,
            description: `Scanned Dust Bin QR (${qrIdForTracking.substring(0, 30)}...)`, // Shorten description for display
            timestamp: new Date().toISOString()
        });

        users[userIndex] = currentUser; // Update the user object in the all users array
        saveUsersToLocalStorage(users); // Save the updated users array to local storage
        ...
        showToast("Error processing QR scan.", 'error');
...
async function redeemCoupon(couponId, pointsRequired) {
    ...
    showLoading();
    try {
        // --- Simulate backend logic for coupon redemption ---
        let users = loadUsersFromLocalStorage();
        let userIndex = users.findIndex(u => u.username === currentUser.username);
        ...
        currentUser.points -= coupon.points;
        currentUser.redeemedCoupons.push({
            coupon_id: coupon.id,
        ...
        users[userIndex] = currentUser; // Update user in the all users array
        ...
        showToast("Error during coupon redemption.", "error");
...
function renderPointsHistory(historyData, redeemedData) {
    ...
    const allActivities = [
        ...
        ...(currentUser.redeemedCoupons || []).map(item => ({ // Include redeemed coupons
            type: 'redeem',
            timestamp: item.redeemed_at,
            points: -item.points_used, // Already negative, but ensure for consistency
